...
export default function VendorDashboard() {
  return (
    <div className="bg-gray-100 flex min-h-screen overflow-hidden w-full">
      {/* Sidebar */}
      <aside className="w-64 bg-pink-700 text-white flex-col hidden md:flex h-screen sticky top-0">
        <div className="p-6 text-2xl font-bold border-b border-pink-500 text-center">
          زغروطة للأعمال ✨
        </div>
        <nav className="flex-1 p-4 space-y-2">
          <Link href="/vendor-dashboard" className="block p-3 rounded-xl bg-pink-800 font-bold flex items-center gap-3">
            <LayoutDashboard size={20} /> الرئيسية
          </Link>
          <a href="#" className="block p-3 rounded-xl hover:bg-pink-600 transition flex items-center gap-3">
        ...
        <div className="p-4 border-t border-pink-500">
          <Link href="/" className="block p-3 text-center bg-pink-900 rounded-xl hover:bg-red-900 transition text-sm flex items-center justify-center gap-2">
            <LogOut size={16} /> خروج
          </Link>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col overflow-y-auto w-full min-h-screen pb-12 md:pb-0">
        <header className="bg-white shadow-sm p-4 flex justify-between items-center px-4 md:px-8 sticky top-0 z-10">
          ...
          <div className="flex items-center gap-4">
            <span className="relative text-gray-500 hover:text-pink-600 cursor-pointer">
              <Bell size={24} />
            ...
            <div className="w-10 h-10 bg-pink-200 rounded-full border-2 border-pink-500"></div>
          </div>
        </header>
        ...
        <div className="p-4 md:p-8 space-y-8">
          ...
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100">
            <div className="p-6 border-b flex justify-between items-center">
              ...
              <button className="text-pink-600 text-sm hover:underline font-semibold">عرض الكل</button>
            </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 w-full bg-pink-700 text-white z-50 px-4 py-2 flex justify-between items-center shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
